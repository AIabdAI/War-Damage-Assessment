...
- تدير الأقفال (Locks) محلياً عبر processed_log.json (المزامنة مع GitHub أُزيلت).
- تحفظ الـ labels في data/annotations محلياً فقط — إدارة git/DVC تتم يدوياً
  خارج الأداة وفق سير عمل المشروع (DVC يملك البيانات، git يملك الكود).
- Idempotency عبر MD5 + سجل الصور المعالجة.
- تصدير إحصائيات YAML وتقرير Markdown محلياً (بدون فروع أو Pull Requests).
...
المتطلبات:
    pip install -r requirements.txt   (تتضمن Pillow و PyYAML)
"""
...
class LockManager:
    ...
    # ------------- محلي فقط (مزامنة git/GitHub أُزيلت من الأداة) -------------

    def refresh_from_remote(self) -> None:
        """كانت تجلب السجل من GitHub — الآن السجل محلي فقط."""
        prune_expired_locks(self.log)

    def sync(self, message: str, extra_paths: list[Path] | None = None) -> tuple[bool, str]:
        """كانت تزامن السجل والملفات مع GitHub (fetch → merge → commit → push).
        أُزيلت المزامنة: تحفظ السجل محلياً فقط، وإدارة git/DVC تتم يدوياً من
        الطرفية وفق سير عمل المشروع."""
        prune_expired_locks(self.log)
        write_log(self.log_path, self.log)
        return True, "تم الحفظ محلياً — المزامنة مع GitHub/DVC أُزيلت من الأداة."
...
def publish_report_pr(lm: LockManager, report_md: str, stats_path: Path) -> str:
    """يكتب التقرير محلياً فقط (إنشاء الفروع وفتح الـ PR أُزيلا من الأداة)."""
    reports = lm.repo / REPORTS_DIR
    reports.mkdir(parents=True, exist_ok=True)
    report_path = reports / "annotation_report.md"
    report_path.write_text(report_md, encoding="utf-8")
    return ("تم حفظ التقرير محلياً:\n"
            f"{report_path}\n{stats_path}\n"
            "ارفعهما عبر git يدوياً عند الحاجة.")

# ----------------------------------------------------------------------------
# بديل محلي: تكامل DVC أُزيل من الأداة — الرفع/السحب يتم يدوياً من الطرفية
# (dvc add data/raw && dvc push  /  dvc pull) وفق playbook المشروع.
# ----------------------------------------------------------------------------

class DvcManager:
    """واجهة متوافقة مع بقية الكود لكن بدون أي أمر dvc أو git."""

    def __init__(self, repo: Path):
        self.repo = repo

    def available(self) -> bool:
        return True   # لا يمنع أي ميزة محلية (الحذف المحلي مثلاً)

    def raw_state(self) -> str:
        return "clean"   # لا فحص عن بُعد

    def raw_changed(self) -> bool:
        return False

    def pull_raw(self, state=None) -> tuple[bool, str]:
        return True, "تكامل DVC أُزيل من الأداة — استخدم dvc pull يدوياً عند الحاجة."

    def sync_raw(self, lm=None) -> tuple[bool, str]:
        return True, "تكامل DVC أُزيل من الأداة — استخدم dvc add/push يدوياً."

    def delete_image_from_dvc(self, image_path: Path) -> tuple[bool, str]:
        """حذف محلي فقط لملف الصورة — بدون أي أمر dvc."""
        if not image_path.exists():
            return False, f"ملف الصورة غير موجود: {image_path.name}"
        try:
            image_path.unlink()
        except OSError as e:
            return False, f"تعذّر حذف ملف الصورة (قد يكون مفتوحاً في برنامج آخر): {e}"
        return True, ("تم حذف الصورة محلياً. حدّث DVC يدوياً لاحقاً: "
...
class AnnotationApp:
    def __init__(self, root: tk.Tk, repo: Path, user: str,
        ...
        if not has_images:
            messagebox.showwarning(
                "الصور", "مجلد data/raw فارغ — ضع الصور فيه ثم أعد التشغيل.\n"
                          "(المزامنة عبر DVC أُزيلت من الأداة — عند الحاجة: dvc pull يدوياً)")
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="📄 تقرير محلي", command=self.report_pr).pack(side=tk.RIGHT, padx=2)
    ...
    def delete_current_image(self):
        ...
        if not messagebox.askyesno(
                ...
                f"الحذف محلي فقط — تذكّر تحديث DVC يدوياً لاحقاً: dvc add data/raw && dvc push.\n\n"
                f"متابعة الحذف؟"):
            return
        # تأكيد ثانٍ صريح — العملية تمسّ الجميع
        if not messagebox.askyesno("تأكيد أخير",
                                   f"تأكيد حذف «{img_path.name}» نهائياً من هذا الجهاز؟"):
            return
    ...
    def report_pr(self):
        ...
        def worker():
            ...
            self.root.after(0, lambda: (self._update_status(),
                                        messagebox.showinfo("تقرير محلي", msg)))
